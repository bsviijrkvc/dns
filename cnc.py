import ipaddress, time, sys, threading, socket, subprocess, random, os
    ...
    while 1:
        try:
            ...
            if command == 'HELP' or command == '?' or command == 'COMMANDS':
                ...
                send(client, f'\n{B}Instagram{gray}: {Q}nt00')
                send(client, f'{B}Telegram{gray}: {Q}nt00')
                send(client, f'{B}Discord{gray}: {Q}nt00')
                send(client, f'{B}GitHub{gray}: {Q}nt00\n')
